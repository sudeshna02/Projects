    ...
    <div class="row mb-5">
        <div class="col">
            <h3>
                ...
                ><span class="text-muted small"> Users</span><span class="text-uppercase text-danger"> /</span><span class="text-muted small"> View - Patients <i class="fas fa-user-injured mr-3 text-muted"></i></span>
            </h3>
        </div>
    </div>
    <!-- end of title row -->
 ...
 <!--################## main content ################-->
                ...
                <div class="card_body">
                    <div class="row d-flex">
                        ...
                        <span class="d-inline-block" data-toggle="popover" data-content="You cannot add a patient!">
                          <button class="btn btn-info" style="pointer-events: none;" type="button" disabled><i class="bi bi-person-plus"></i> Add A Patient</button>
                        </span>
                        ...
                        <table style="width:100%;" id="filtertable" class="table cust-datatable dataTable no-footer">
                            ...
                            <tbody>
                              <?php foreach ($requests as $diary) { ?>
                                <tr>
                                    <td><?= $diary['pfirstname'] . ' ' . $diary['pmiddlename'] . ' ' . $diary['psurname'] ?></td>
                                    <td><?= $diary['pcontact'] ?></td>
                                    <td><?= $diary['pemail'] ?></td>
                                    ...
                                    <td>
                                         <?php if($diary['pstatus'] == '2'){?>   
                                            <button type="submit" class="btn btn-dark disablePatient" patient-id="<?= $diary['pid'] ?>">Disable <i class="bi bi-x-circle"></i></button>
                                         <?php } 
                                        else { ?>
                                            <button type="submit" class="btn btn-success enablePatient" up-id="<?= $diary['pid'] ?>">Enable <i class="bi bi-check-circle"></i></button>
                                         <?php }  ?>                                  
                                    </td>

                                    <td>                      
                                         <button type="submit" class="btn btn-info editPatientUser"

                                         user-first = "<?=$diary['pfirstname']?>"
                                         user-midname = "<?=$diary['pmiddlename']?>"
                                         user-surname = "<?=$diary['psurname']?>"
                                         user-email = "<?=$diary['pemail']?>"
                                         user-phone = "<?=$diary['pcontact']?>"
                                         user-meet = ""
                                         user-name = "<?=$diary['pusername']?>"
                                         userr-id = "<?= $diary['pid'] ?>"

                                         >
                                                Edit <i class="bi bi-pencil"></i>
                                        </button>
                                        <button type="submit" class="btn btn-danger deletePatient" upa-id="<?= $diary['pid'] ?>">
